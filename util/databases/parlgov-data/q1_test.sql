...
/* -- Country, year pair which had more than one election that year
CREATE OR REPLACE VIEW MultipleInstances AS
SELECT id, country_id, e_year, votes_valid, party_id, votes, party_range
FROM (SELECT country_id, e_year
      FROM CrossedElections
      GROUP BY country_id, e_year
HAVING count(DISTINCT id) > 1) as MultE JOIN CrossedElections USING(country_id, e_year);


-- Removes all instances from CrossedElections related to MultipleInstances
CREATE OR REPLACE VIEW OnlyOneElec AS
(SELECT * FROM CrossedElections)
EXCEPT ALL
(SELECT * FROM MultipleInstances); */